...
const generatedOutputDir = path.join(__dirname, '..', 'generated', 'api');
const serviceOutputDir = path.join(
  __dirname,
  '..',
  'generated',
  'api',
...
if (fs.existsSync(serviceOutputDir)) {
  fs.rmSync(serviceOutputDir, { recursive: true });
}
...
function removeApiFiles() {
  const apiModulePath = path.join(generatedOutputDir, 'api.module.ts');
  const apiConfigurationPath = path.join(
    generatedOutputDir,
    'api-configuration.ts',
  );
  const baseServicePath = path.join(generatedOutputDir, 'base-service.ts');
  const modelsPath = path.join(generatedOutputDir, 'models.ts');
  const requestBuilderPath = path.join(
    generatedOutputDir,
    'request-builder.ts',
  );
  const servicesPath = path.join(generatedOutputDir, 'services.ts');
  const strictHttpResponsePath = path.join(
    generatedOutputDir,
  ...
  [
    ...
    requestBuilderPath,
    servicesPath,
    strictHttpResponsePath,
  ].forEach((filePath) => {
    if (fs.existsSync(filePath)) {
      fs.unlinkSync(filePath);
    }
  });
  getFilesByExtension(serviceOutputDir, '.service.ts').forEach((file) => {
    fs.unlinkSync(join(serviceOutputDir, file));
  });
}
  ...
  Object.entries(services).forEach(
    ([serviceName, { operations, modelImports }]) => {
      const importStatements = Array.from(modelImports)
        .map(
          (modelName) =>
            `import { ${modelName} } from '../models/${toKebabCase(
              modelName,
            )}';`,
        )
        .join('\n');
...
`;
      ...
      fs.writeFile(outputFilePath, outputContent, 'utf8', (writeErr) => {
        ...
        console.log(`Processed: ${serviceName}:`, outputFilePath);
      });
    },
  );
});
